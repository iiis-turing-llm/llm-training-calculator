...
const DETAIL_COLOR_MAPPING: any = {
    ...
    forward_reduce: {
        ...
        label: 'Per-loop forward reduce_ scatter time(s)',
        color: '#9A60B4'
    },
    // backward: {
...
const PanelRightPop: FC<IPanelRightPopProps> = (props) => {
    ...
    const { forward_time, num_microbatches, forward_gpu_usage,
    ...
    return <div className={styles.pop_wrapper}>
        ...
        {!noPCIE() && <div className={styles.pop_chart_legend}>
            ...
            <span>
                <b>{DETAIL_COLOR_MAPPING['forward_reduce'].category}</b>
                {DETAIL_COLOR_MAPPING['forward_reduce'].label}
            </span>
        </div>}
