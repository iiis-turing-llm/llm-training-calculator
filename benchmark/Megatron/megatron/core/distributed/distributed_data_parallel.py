...
class DistributedDataParallel(MegatronModule):
    ...
    def __init__(
        ...
        """
        Allocate separate grad fields, remove grad buffers
        """
        for dtype, params in grad_dtype_to_params.items():
            for param in params[::-1]:
                param.main_grad = torch.zeros(param.data.shape, dtype=dtype, device=torch.cuda.current_device())
    ...
    def _make_param_hook(
        ...
        def param_hook(*unused):
            if param.requires_grad:
                ...
                param.grad = None
                """
                No grad buffer so no "param_to_grad_buffer" variable
                """
                # if self.overlap_grad_reduce:
