...
class DistributedOptimizer(MixedPrecisionOptimizer):
    ...
    def build_optimizer_group_ranges(cls, param_groups, model_gbuf_ranges):
        ...
        """
        Divide the main_grad fields in roughly equal-sized parts, remove original mapping
        """
        ...
        for index, param_group in enumerate(param_groups):
            group_range = group_ranges[index]
            for r in range(data_parallel_world_size)[::-1]:
                for param in param_group["params"][len(param_group["params"]) * r // data_parallel_world_size:len(param_group["params"]) * (r + 1) // data_parallel_world_size][::-1]:
                    param.index = r
                    if r == data_parallel_rank:
                        group_range["params"].append(param)
                        local_param_group_map[param] = \
                            (group_index, len(group_range["params"]) - 1)
        """
        ...
        for group_index, group_range in enumerate(opt_group_ranges):
            ...
            for model_param in group_range["params"]:
                ...
                if model_param.type() in ['torch.cuda.HalfTensor',
                    ...
                    # Clone model -> main.
                    shard_model_param = model_param.detach().view(-1) # no need to shard
    ...
    def __init__(self, optimizer, clip_grad, log_num_zeros_in_grad,
        ...
        self.data_parallel_rank = mpu.get_data_parallel_rank(with_context_parallel=True) # get ranks for future computation
        self.data_parallel_world_size = mpu.get_data_parallel_world_size(with_context_parallel=True) # get ranks for future computation
        ...
        def copy_group_grads(model_groups, shard_main_groups):
            for model_group, shard_main_group in zip(model_groups,
                ...
                for model_param, shard_main_param in zip(model_group,
                                                         shard_main_group):
                    """
                    No param range map, directly copy the main_grad field
                    """
        ...
        def copy_group_params(shard_main_groups, model_groups):
            for shard_main_group, model_group in zip(shard_main_groups,
                ...
                for shard_main_param, model_param in zip(shard_main_group,
                    ...
                    """
                    No param range map, directly copy the params
                    """
                    '''param_range_map = self.get_model_param_range_map(model_param)
                    ...
                    No param range map, directly copy the params
                    """
                    model_param.data.copy_(shard_main_param.view_as(model_param).to(model_param.device))
        ...
        if not self.overlap_param_gather:
            ...
            for model_chunk in self.models:
                for param in model_chunk.parameters():
                    if param.index not in group_params:
                        group_params[param.index] = [param.data]
                    else:
                        group_params[param.index].append(param.data)
            for rank, tensors in group_params.items():
                torch.cuda.empty_cache()
                if rank == 0:
                    with open("zero_2.txt", "a") as file:
                        file.write(f"{torch.distributed.get_rank()}, " + str(torch.cuda.memory_allocated())+"\n")
                temp_tensor = torch._utils._flatten_dense_tensors(tensors).to(torch.cuda.current_device())
                global_rank = torch.distributed.get_global_rank(group=mpu.get_data_parallel_group(with_context_parallel=True), group_rank=rank)
                torch.distributed.broadcast(temp_tensor, src=global_rank, group=mpu.get_data_parallel_group(with_context_parallel=True))
                start_idx = 0
                torch.cuda.empty_cache()
                if rank == 0:
                    with open("zero_2.txt", "a") as file:
                ...
                for t in tensors:
                    ...
                    t.copy_(temp_tensor[start_idx:end_idx].view_as(t).to(t.device))
                    start_idx = end_idx
                del temp_tensor
            """
            All gather
